...
file_name <- paste(dl_folder, "data", "bronze", "ec", paste0("building_subsectors_nsa_nace2", ".zip"), sep = "/")
...
tfile <- tempfile(tmpdir = tdir <- tempdir())
file_name <- paste(dl_folder, "data", "bronze", "ec", paste0("building_subsectors_sa_nace2", ".zip"), sep = "/")
zipped_files <- unzip(file_name, exdir = tdir)
