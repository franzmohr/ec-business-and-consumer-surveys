...
file_name <- paste(dl_folder, "data", "bronze", "ec", paste0("consumer_subsectors_nace2", ".zip"), sep = "/")
...
unlink(tdir)
rm(list = c("i", "m_nsa", "q_nsa", "temp", "sheets", "file_name"))

# Seasonally adjusted ----

## Monthly data ----
file_name <- zipped_files[which(grepl("_sa_m_nace2", zipped_files))]
sheets <- excel_sheets(file_name)
sheets <- sheets[!sheets %in% c("Index", "INFO")]
...
file_name <- zipped_files[which(grepl("_sa_q_nace2", zipped_files))]
sheets <- excel_sheets(file_name)
sheets <- sheets[!sheets %in% c("Index", "INFO")]
...
unlink(tdir)
rm(list = c("i", "m_sa", "q_sa", "temp", "sheets", "file_name", "zipped_files", "tdir", "tfile"))
