...
file_name <- paste(dl_folder, "data", "bronze", "ec", paste0("retail_total_nsa_nace2", ".zip"), sep = "/")
zipped_files <- unzip(file_name, exdir = tdir)

# excel_sheets(zipped_files)
monthly_data <- read_excel(zipped_files, "RETAIL TRADE MONTHLY", na = "NA")
...
tfile <- tempfile(tmpdir = tdir <- tempdir())
file_name <- paste(dl_folder, "data", "bronze", "ec", paste0("retail_total_sa_nace2", ".zip"), sep = "/")
zipped_files <- unzip(file_name, exdir = tdir)
