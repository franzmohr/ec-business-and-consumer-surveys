...
file_name <- paste(dl_folder, "data", "bronze", "ec", paste0("services_subsectors_nsa_nace2", ".zip"), sep = "/")
...
unlink(tdir)
rm(list = c("m_nsa", "q_nsa", "m_sa", "q_sa",
            "temp", "sheets", "file_name", "zipped_files", "tdir", "tfile"))

# Seasonally adjusted ----

tfile <- tempfile(tmpdir = tdir <- tempdir())
file_name <- paste(dl_folder, "data", "bronze", "ec", paste0("services_subsectors_sa_nace2", ".zip"), sep = "/")
zipped_files <- unzip(file_name, exdir = tdir)
