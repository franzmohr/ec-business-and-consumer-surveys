...
file_name <- paste(dl_folder, "data", "bronze", "ec", paste0("industry_total_nsa_nace2", ".zip"), sep = "/")
zipped_files <- unzip(file_name, exdir = tdir)

# excel_sheets(zipped_files)
monthly_data <- read_excel(zipped_files, "INDUSTRY MONTHLY", na = "NA")
names(monthly_data)[1] <- "date"

quarterly_data <- read_excel(zipped_files, "INDUSTRY QUARTERLY", na = "NA")
...
tfile <- tempfile(tmpdir = tdir <- tempdir())
file_name <- paste(dl_folder, "data", "bronze", "ec", paste0("industry_total_sa_nace2", ".zip"), sep = "/")
zipped_files <- unzip(file_name, exdir = tdir)
